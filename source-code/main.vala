...
public class HackerOSStore : Gtk.Application {
    ...
    private async void install_proton_ge_async() throws HackerError {
        ...
        bool success = false;
        try {
            success = Process.spawn_command_line_sync(@"tar -xzf $tar_file -C $proton_dir --strip-components=1", null, null, out status);
        } catch (SpawnError e) {
            throw new HackerError.FAILED("Failed to extract Proton-GE: " + e.message);
        }
        if (!success || status != 0) throw new HackerError.FAILED("Failed to extract Proton-GE");
    ...
    private async string get_proton_ge_url() throws Error {
        var session = new Session();
        session.user_agent = "Mozilla/5.0";
        var message = new Message("GET", "https://api.github.com/repos/GloriousEggroll/proton-ge-custom/releases/latest");
        ...
        if (message.status_code != 200) {
            throw new HackerError.FAILED(@"Failed to get Proton-GE info: HTTP $(message.status_code)");
        }
        var parser = new Json.Parser();
        parser.load_from_data((string)bytes.get_data());
        var root = parser.get_root().get_object();
        var assets = root.get_array_member("assets");
        foreach (unowned Json.Node node in assets.get_elements()) {
            var obj = node.get_object();
            var name = obj.get_string_member("name");
            if (name.has_suffix(".tar.gz")) {
                return obj.get_string_member("browser_download_url");
            }
        }
        throw new HackerError.FAILED("No Proton-GE tar.gz found");
    }
